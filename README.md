...
The releases contain zero verified shopper episodes and zero instrumented agent transactions. Four sourced Adobe benchmarks remain survey and referral context, and the Google Trends series remains a separate normalized search-interest lane. The newest method pilot adds 11 reviewed signed-in app attempts from four buyer intents, including three retries, seven completed two-turn answers, six clean selections, one contradiction, and four failures. A ten-task provider-API check is reported on its own surface. `GET /ai-commerce/selection` exposes the app labels and private-archive digests, while raw personal chats stay private. This convenience sample does not support Agent Selection Share™ or a product quality ranking. The previous broad wallet packets and NKE/TJX instrument have been removed from the active tree; legacy API routes return HTTP 410 with a successor URL, and the old Webflow page URLs hand off to the new pages. No paid daily collection is scheduled. See [method and operations](docs/agentic-commerce-signal-design.md).
