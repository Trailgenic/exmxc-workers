...
ok(tools.length === 10, "tools/list returns 10 tools");
ok(tools.includes("ex.eei.audit.run"), "tools/list includes ex.eei.audit.run");
ok(tools.includes("ex.convergence.latest") && tools.includes("ex.convergence.log"), "tools/list includes convergence tools");
...
const cvLatest = await rpc(7, "tools/call", { name: "ex.convergence.latest", arguments: {} });
const cvLatestData = JSON.parse(cvLatest.d.result.content[0].text);
ok(cvLatestData?.latest?.status && typeof cvLatestData.latest.breaches === "number", "ex.convergence.latest returns a read");
const cvLog = await rpc(8, "tools/call", { name: "ex.convergence.log", arguments: { limit: 1 } });
const cvLogData = JSON.parse(cvLog.d.result.content[0].text);
ok(Array.isArray(cvLogData?.log) && cvLogData.log.length === 1, "ex.convergence.log respects limit");
...
ok(reg.d?.registry_version === "2.0" && regIds.length === 10, "tool-registry has version 2.0 and 10 tools");
