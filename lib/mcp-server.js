...
import { getAiPowerIndex, getAiPowerProfilesV2, getDatasetIndex, getEntities, getFourForces, getIndex, getSpeg, getSpegIndexV1, TOOL_HANDLERS } from "./queries.js";
...
export function resolveMcpResource(uri) {
  ...
  if (resource.resolver === "entities") return getEntities({});
  if (resource.resolver === "speg") return getSpeg({});
  if (resource.resolver === "spegIndexV1") return getSpegIndexV1({});
  if (resource.resolver === "aiPowerIndexLegacy") return getAiPowerIndex();
  if (resource.resolver === "fourForcesLegacy") return getFourForces();
  if (resource.resolver === "aiPowerProfilesV2") return getAiPowerProfilesV2({});
