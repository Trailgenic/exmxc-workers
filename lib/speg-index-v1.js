...
export const SPEG_INDEX_RELEASE_ID = "speg-index-2026-09-17-rc1";
...
function dimensionRecord(profile, key, context) {
  ...
  return {
    ...
    source_ids: [...DIMENSION_SOURCE_IDS[profile.issuer_id][key]],
...
function buildProfile(profile, sources) {
  const context = PROFILE_CONTEXT[profile.issuer_id];
  if (!context) throw new Error(`Missing sPEG Index context for ${profile.issuer_id}`);
  ...
  return {
    ...
    dimensions: Object.fromEntries(DIMENSION_KEYS.map((key) => [key, dimensionRecord(profile, key, context)])),
    score: {
    ...
    review: {
      ...
      reviewed_at: `${profile.reviewed_on}T23:59:59Z`,
    ...
    release: {
      state: "draft",
      release_id: SPEG_INDEX_RELEASE_ID,
      predecessor_release_id: null,
      change_reasons: ["initial_research_constitution"],
      content_hash: null
    },
    valuation: {
      peg_eligible: false,
...
function buildFullRelease() {
  const sourcesById = Object.fromEntries(Object.entries(authoredRelease.sources).map(([id, source]) => [id, sourceRecord(id, source)]));
  const profiles = authoredRelease.profiles.map((profile) => buildProfile(profile, sourcesById));
  ...
  return {
    ...
    links: {
      ...
      release: `${API_ORIGIN}/speg/index/v1/releases/${SPEG_INDEX_RELEASE_ID}`,
...
export const SPEG_INDEX_RELEASE = buildFullRelease();

function sourceSubset(profiles) {
  const ids = new Set(profiles.flatMap((profile) => DIMENSION_KEYS.flatMap((key) => profile.dimensions[key].source_ids)));
  return SPEG_INDEX_RELEASE.sources.filter((source) => ids.has(source.id));
}

function filterRelease(profiles) {
  const sources = sourceSubset(profiles);
  return {
    ...structuredClone(SPEG_INDEX_RELEASE),
    coverage: releaseCoverage(profiles),
    profiles: structuredClone(profiles),
    sources: structuredClone(sources),
    provenance: {
      ...SPEG_INDEX_RELEASE.provenance,
      source_count: sources.length,
      source_origin_groups: [...new Set(sources.map((source) => source.origin_group))].sort()
    }
  };
...
function suggestions(query) {
  const needle = normalizeText(query);
  if (!needle) return [];
  return SPEG_INDEX_RELEASE.profiles
...
export function getSpegIndexV1(args = {}) {
  ...
  if (args.release && args.release !== SPEG_INDEX_RELEASE_ID) {
    return { found: false, error: "Unknown sPEG Index release.", release_id: args.release };
  }
  let profiles = [...SPEG_INDEX_RELEASE.profiles];
  ...
  if (args.query && profiles.length === 0) {
    return { found: false, error: "Unknown sPEG Index issuer.", query: args.query, suggestions: suggestions(args.query) };
  }
...
export function getSpegIndexReleasesV1() {
  return {
    product: PRODUCT,
    method_id: SPEG_INDEX_METHOD_ID,
    latest_published_release_id: null,
    draft_release_ids: [SPEG_INDEX_RELEASE_ID],
    releases: [{
      release_id: SPEG_INDEX_RELEASE_ID,
      state: "draft",
      research_cutoff: SPEG_INDEX_RELEASE.research_cutoff,
      publication_at: null,
      effective_at: null,
      predecessor_release_id: null,
      content_hash: null,
...
export function validateSpegIndexReleaseSemantics(release) {
  ...
  if (release.release_state === "released") {
    ...
    if (release.profiles.some((profile) => profile.decision.membership_state === null)) errors.push("released profiles require a membership state");
  }
