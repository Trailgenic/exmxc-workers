...
export function getAiCommerceSelection({ release } = {}) {
  const result = getAiCommerceSignal({ release });
  if (result.found === false || !result.selection_panel) return { found: false, error: 'No selection panel in this AI commerce release.' };
  return { release_id: result.release_id, as_of: result.as_of, selection_panel: result.selection_panel, api_method_check: result.api_method_check };
}
...
export function buildAiCommerceRelease(episodes, { releaseId, asOf, benchmarks = [], searchInterest, selectionPanel, apiMethodCheck, selectionRunCount = 0, notes = '' }) {
  ...
  return {
    ...
    benchmarks, ...(searchInterest ? { search_interest: searchInterest } : {}),
    ...(selectionPanel ? { selection_panel: selectionPanel } : {}),
    ...(apiMethodCheck ? { api_method_check: apiMethodCheck } : {}), notes
  };
}
