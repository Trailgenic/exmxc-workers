...
export async function collectConsumerIntent(options) {
  ...
  for (const target of collectionPlan.targets) {
    const discovery = await callConsumerWebSearch(consumerDiscoveryPrompt(target, collectionDate), options);
    ...
    for (let index = 0; index < candidates.length; index += 1) {
      ...
      else rejected.push({ target_id: target.id, candidate_id: candidate.candidate_id, reasons: checked.reasons });
    }
    let verifiedCount = 0;
    if (preverified.length) {
      const verification = await callConsumerWebSearch(consumerVerificationPrompt(preverified.map(({ candidate }) => candidate), collectionDate), options);
      const verificationUrls = consumerSearchSources(verification);
      ...
      for (const item of preverified) {
        ...
        const cited = canonicalConsumerSourceUrl(item.candidate.source_url);
        if (check?.verified === true && verificationUrls.has(cited)) {
          accepted.push(item.observation);
          verifiedCount += 1;
        } else {
          rejected.push({ target_id: target.id, candidate_id: item.candidate.candidate_id, reasons: [check?.reason || "independent_verification_failed"] });
        }
      }
    }
