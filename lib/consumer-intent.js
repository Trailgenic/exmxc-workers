...
import foundationRelease from "../data/consumer_intent_v1/releases/2026-09-21-foundation.json" with { type: "json" };
...
export const CONSUMER_INTENT_LATEST = foundationRelease;

export const CONSUMER_INTENT_RELEASES = {
  series_id: "consumer-intent-pulse-v1",
  latest: foundationRelease.release_id,
  releases: [
    {
      release_id: foundationRelease.release_id,
      as_of: foundationRelease.as_of,
      status: foundationRelease.release_status,
      methodology_id: foundationRelease.methodology_id,
      observation_count: foundationRelease.coverage.observation_count,
      label: "Foundation · September 2026"
    }
  ]
};
...
export function aggregateConsumerFactor(observations, factor, window = "30d") {
  ...
  const sources = new Set(unique.map((row) => `${row.source_type}:${row.source}`));
  ...
  const concentration = unique.length
    ? Math.max(...[...sources].map((source) => unique.filter((row) => `${row.source_type}:${row.source}` === source).length)) / unique.length
    : 1;
...
export function getConsumerIntentPulse(args = {}) {
  const release = args.release && args.release !== foundationRelease.release_id ? null : foundationRelease;
...
export function getConsumerIntentEntity(value) {
  ...
  const reading = foundationRelease.entity_readings.find((row) => row.entity_id === parentId) ?? null;
  return {
    found: true,
    release_id: foundationRelease.release_id,
    as_of: foundationRelease.as_of,
    ...
    interpretation_boundary: foundationRelease.interpretation_boundary
  };
}
