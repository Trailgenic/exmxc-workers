...
export function validateEvidenceDocumentSnapshot(profile, manifest, snapshot) {
  ...
  if (snapshot.snapshot_version !== "ai-power-source-snapshot-v1") errors.push("Evidence snapshot version is unsupported.");
...
export function assembleVerifiedProfile(baseRelease, entityId, documents, verified, assessedAt) {
  ...
  for (const claim of verified.claims || []) {
    ...
    const locator = nonemptyString(claim.locator);
    const extract = nonemptyString(claim.extract);
    const criterionIds = validCriterionIds(claim.criterion_ids);
    const exactExtract = document && extract && normalizeExtract(document.text).includes(normalizeExtract(extract));
    ...
    const evidenceFingerprint = createHash("sha256")
      .update(JSON.stringify([document.id, normalizeExtract(extract)]))
      .digest("hex")
      .slice(0, 16);
