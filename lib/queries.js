import { getPowerRankings } from './ai-power-rankings.js';
...
import { getSpegIndexV1 } from "./speg-index-v1.js";
import { getConsumerIntentEntity, getConsumerIntentPulse } from "./consumer-intent.js";
...
export function getStrategicConsequence(args = {}) {
  ...
  return {
    ...
    model_status: "experimental_legacy_v1",
    compatibility_boundary: "This engine is pinned to the legacy v1 Four Forces exposure scaffold. AI Power v2 profile judgments are not used as inputs.",
...
export function getAiPowerIndex() {
  return {
    ...DATASETS.ai_power_index.data,
    metadata: {
      ...DATASETS.ai_power_index.data.metadata,
      status: "legacy_exposure_scaffold",
      current_authority: false,
      interpretation_boundary: "These weighted exposure values are preserved for historical compatibility. They are not evidence-backed AI Power v2 judgments."
    }
  };
}
...
export function getFourForces() {
  return {
    ...DATASETS.four_forces.data,
    metadata: {
      ...DATASETS.four_forces.data.metadata,
      status: "legacy_exposure_scaffold",
      current_authority: false,
      interpretation_boundary: "The v1 exposure scaffold is retained for compatibility with the legacy scenario engine; it is not the AI Power v2 evidence profile."
    }
  };
}
...
export function getAiPowerTop(args = {}) {
  ...
  return {
    analysis: "Legacy AI Power v1 Weighted Exposure Ranking",
    status: "legacy_exposure_scaffold",
    current_authority: false,
    limit,
    results,
    ...
    tie_policy: "Rows are sorted by stored total. Array order within equal scores is presentation order and does not imply different merit.",
    disclaimer: "Historical compatibility output; not an evidence-backed AI Power v2 ranking."
  };
}
...
export function getPowerLens(args = {}) {
  ...
  if (!record) {
    return {
      ...
      version: "1.0",
      model_status: "legacy_exposure_scaffold",
      current_authority: false,
      query,
      found: false,
      generated_at: BUILD.released,
      ...
      coverage: {
        ...
        scope: "Bundled exmxc legacy v1 weighted exposure universe",
        dynamic_company_generation: false
      },
      disclaimer: "No score was generated. Legacy Power Lens v1 resolves only entities in its bundled weighted exposure universe."
    };
  }
  ...
  return {
    ...
    interpretation: {
      summary: `${record.entity_name} is #${rank} of ${scores.length} in the legacy v1 weighted exposure ordering, with a stored exposure total of ${record.ai_power_index}. This is not an AI Power v2 judgment.`,
    ...
    provenance: {
      methodology: "Legacy v1 exposure weights: Compute 30%, Interface 25%, Alignment 25%, Energy 20%.",
    ...
    suggestions: [],
    disclaimer: "Historical compatibility view based on the v1 weighted exposure scaffold and adjacent bundled datasets. It is not an AI Power v2 profile, live market data, or investment advice."
  };
}
...
export const TOOL_HANDLERS = {
  "ex.consumer_intent.pulse.get": getConsumerIntentPulse,
  "ex.consumer_intent.entity.get": ({ query }) => getConsumerIntentEntity(query),
  "ex.ai_power.rankings.get": getPowerRankings,
  "ex.entities.get": getEntities,
  "ex.speg.get": getSpeg,
  "ex.speg.index.get": getSpegIndexV1,
  "ex.datasets.index.get": getDatasetIndex,
  "ex.ai_power_index.get": getAiPowerIndex,
  "ex.four_forces.get": getFourForces,
  "ex.ai_power.profiles.get": getAiPowerProfilesV2,
  "ex.power_lens.v2.get": getPowerLensV2,
