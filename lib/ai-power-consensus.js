...
function normalizedExtract(item) {
  return item.extract.replace(/\s+/g, " ").trim().toLowerCase();
}

function repeatedPassage(first, second) {
  if (!first || !second
    ...
    || first.source.origin_id !== second.source.origin_id) return false;
  const firstExtract = normalizedExtract(first);
  const secondExtract = normalizedExtract(second);
  if (firstExtract === secondExtract) return true;
  const shorter = firstExtract.length <= secondExtract.length ? firstExtract : secondExtract;
  const longer = firstExtract.length > secondExtract.length ? firstExtract : secondExtract;
  return shorter.length >= 40 && shorter.split(/\s+/).length >= 7 && longer.includes(shorter);
}

function mergeRepeatedEvidence(first, second) {
  if (!repeatedPassage(first, second)) return null;
  const criterionIds = first.criterion_ids.filter((id) => second.criterion_ids.includes(id));
  if (!criterionIds.length) return null;
  const retained = normalizedExtract(first).length <= normalizedExtract(second).length ? first : second;
  return {
    ...retained,
...
function reconcileRepeatedEvidence(firstEvidence, secondEvidence) {
  const unusedSecondIds = new Set(secondEvidence.map((item) => item.id));
  const firstRefMap = new Map();
  const secondRefMap = new Map();
  const evidence = [];
  for (const first of firstEvidence) {
    const candidates = secondEvidence
      .filter((second) => unusedSecondIds.has(second.id) && repeatedPassage(first, second))
      .sort((left, right) => {
        const leftExactId = left.id === first.id ? 1 : 0;
        const rightExactId = right.id === first.id ? 1 : 0;
        if (leftExactId !== rightExactId) return rightExactId - leftExactId;
        return Math.abs(normalizedExtract(first).length - normalizedExtract(left).length)
          - Math.abs(normalizedExtract(first).length - normalizedExtract(right).length);
      });
    for (const second of candidates) {
      const merged = mergeRepeatedEvidence(first, second);
      if (!merged) continue;
      evidence.push(merged);
      firstRefMap.set(first.id, merged.id);
      secondRefMap.set(second.id, merged.id);
      unusedSecondIds.delete(second.id);
      break;
    }
  }
  return { evidence, firstRefMap, secondRefMap };
}
...
function reconcileCriterion(id, first, second, consensusEvidence, firstRefMap, secondRefMap) {
  ...
  const firstConsensusRefs = new Set(first.evidence_refs.map((ref) => firstRefMap.get(ref)).filter(Boolean));
  const secondConsensusRefs = new Set(second.evidence_refs.map((ref) => secondRefMap.get(ref)).filter(Boolean));
  const evidenceRefs = [...firstConsensusRefs]
    .filter((ref) => secondConsensusRefs.has(ref))
...
function reconcileProfile(first, second, consensusEvidence, firstRefMap, secondRefMap) {
...
export function buildRepeatConsensus(firstRelease, secondRelease, assessedAt = new Date().toISOString()) {
  ...
  const repeated = reconcileRepeatedEvidence(firstRelease.evidence, secondRelease.evidence);
  ...
  const profiles = firstRelease.profiles.map((profile) => reconcileProfile(
    profile,
    secondProfiles.get(profile.entity.id),
    evidenceByEntity.get(profile.entity.id) || [],
    repeated.firstRefMap,
    repeated.secondRefMap
  ));
  const release = {
    ...
    published_at: null,
    pipeline: { ...firstRelease.pipeline, pipeline_version: "ai-power-pipeline-v2.2.0-deterministic-passage-consensus" },
    profiles,
    evidence,
    coverage: {
