...
Status: AI commerce v1 foundation, Google Trends search pilot, and signed-in app selection method pilot, 2026-09-25. The broad Consumer Intent pilot and its dated packets were removed from the active tree. Git history retains the old work for audit; the public API returns HTTP 410 for legacy routes, while the two old Webflow page URLs hand off to AI Commerce. The latest release contains seven completed two-turn app selection attempts, but no verified consumer shopping-experience episodes or agent purchases.
...
| [Google Trends](https://trends.google.com/trends/explore?date=today%2012-m&geo=US&q=ChatGPT%20shopping,AI%20shopping%20assistant) | Public chart and archived CSV export | US weekly search interest for the search terms “ChatGPT shopping” and “AI shopping assistant,” jointly normalized to 0–100 within the export. Current incomplete week excluded. It does not measure AI shoppers, consumer sentiment, or agent purchases. The automated [Trends API](https://developers.google.com/search/apis/trends) is limited-access alpha; do not depend on it for the production feed. |
...
The versioned ontology, separate Adobe survey/referral benchmark lane, Google search-interest export, app selection pilot, release contract, and public API are in `data/ai_commerce_v1`, `schema/ai_commerce_*`, and `lib/ai-commerce.js`. The raw Trends CSV and SHA-256 digest live under `data/ai_commerce_v1/search_interest/`; `--search-csv` imports it into a new immutable release. The selection pilot preserves 11 reviewed public labels, raw-capture hashes, displayed modes, and eight original app-intent cells; the private operator-supplied ZIP and conversation URLs are excluded from Git and the API. `scripts/import-ai-commerce-app-pilot.mjs` checks the exact private archive hash before recreating the public labels. The original five-category provider-API pilot belongs to `Trailgenic/exmxc-crawl-worker` on `feat/agent-commerce-phase0`; its ten tasks and product evidence review are summarized separately. A repeat app protocol is frozen there at `agent-commerce/data/phase0-app-repeat-protocol-2026-09-24.md`. App and API results are never pooled.

For a new release, add permission-reviewed shopping episode arrays under `data/ai_commerce_v1/episodes/` or a new versioned selection panel. Validate and build with `npm run build:ai-commerce -- --input path/to/episodes.json --as-of YYYY-MM-DD --release-id ai-commerce-YYYY-MM-DD-pilot --promote true` or pass `--selection-panel path/to/panel.json`. New selections require full original capture, a declared app surface and mode, stable buyer-intent and protocol hashes, all attempts, final answer labeling, and independent product checks. The current `selection_run_count` means completed two-turn signed-in app attempts only; all 11 physical attempts and failures are also disclosed. Do not schedule collection until access, acceptance rate, repeatability, and category coverage pass review. Do not report an Agent Selection Share™ from this convenience panel.
