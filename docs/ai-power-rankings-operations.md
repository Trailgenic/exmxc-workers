...
- Webflow: `https://www.exmxc.ai/ai-power-index` and `/ai-power-index-methodology`.
...
4. Append a ledger entry with the new file's SHA-256 hash and point `latest` to it. Keep the methodology hash unchanged. Run `npm run build:ai-power-rankings`; it generates the edition imports and all four Webflow code blocks from the same source.
...
7. Read current page code before changing it. Replace only the index and methodology page head/footer blocks with the generated files, preserving site-wide code and unrelated pages. Update matching SEO/OG settings. Publish the scoped pages where supported. Verify public HTML, all three sorts, edition history, and metadata.
...
Reconfirm these through the connector when resuming. The existing v2 Power Lens is a separate historical pilot; this change does not repurpose its old scoring pipeline. No scheduled paid model assessment is needed for rendering or sorting.
