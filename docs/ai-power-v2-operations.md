...
7. Single-entity mode supports development and diagnosis. The `AI Power Pilot Assessment` workflow is a two-entity ASML and Microsoft canary pinned to `gpt-5.6-luna` with Medium reasoning. A committed change to `data/ai_power_v2/canary-trigger.json` records and launches an explicitly authorized canary; ordinary pushes do not invoke the model. The separate cohort runner reads all 20 manifests and records an automatic abstention when collection or model verification fails for one entity instead of aborting the cohort.
