...
3. Run the source snapshot preflight. It retrieves every declared document once without invoking a model, records delivery failures, requires at least two distinct delivered origins for each entity before cohort spending is authorized, and writes an immutable run-scoped source packet for both repeat attempts. Snapshot v2 deterministically segments retained text into passages of at most 900 normalized characters; every passage identifier includes its source id, ordinal, and content hash, and validation rejects any catalog that does not reproduce from the retained text.
...
5. The first model call extracts bounded claims by selecting supplied passage identifiers; it does not generate quote boundaries. The second independently verifies entity, scope, dates, the cited passage identifiers, and contradictions against the same catalog.
6. Deterministic assembly accepts only verified identifiers that belong to the declared source and retrieves the canonical locator and extract from the immutable catalog. Model-generated locator or extract text has no effect on evidence identity. A grade requires Moderate or High confidence, a canonical methodology anchor, supporting evidence, at least one current claim, two distinct evidentiary origins across the graded profile, and at least one primary source. If a proposed grade misses a profile-wide gate, the assembler preserves verified contextual evidence but replaces the grade with `unknown`.
7. Single-entity mode supports development and diagnosis. Subset mode accepts an explicit comma-separated list of pilot entity ids for bounded calibration. The `AI Power Pilot Assessment` workflow uses subset mode, one immutable source snapshot, and two repeat attempts pinned to `gpt-5.6-luna` with Medium reasoning. A committed change to `data/ai_power_v2/canary-trigger.json` records and launches an explicitly authorized calibration; ordinary pushes do not invoke the model. The separate cohort runner reads all 20 manifests and records an automatic abstention when collection or model verification fails for one entity instead of aborting the cohort.
8. A subset or cohort repeat run produces two independent candidate attempts against the same immutable source snapshots, then reconciles them conservatively. Evidence survives only when both attempts select the same deterministic passage from the same source snapshot. A grade survives only when both attempts select the same anchor and the repeated evidence passes the ordinary anchor-support, freshness, source-diversity, and primary-evidence gates. Disagreement becomes `unknown`; model agreement is a repeatability control, not factual corroboration.
...
npm run assess:ai-power -- --entities company-tsmc,company-nextera-energy --manifest-dir ./manifests --dry-run
npm run check:ai-power-sources -- --manifest-dir data/ai_power_v2/manifests
node scripts/snapshot-ai-power-sources.mjs --entities company-tsmc,company-nextera-energy --manifest-dir ./manifests --output-dir ./source-snapshots > source-preflight.json
...
The targeted calibration currently covers TSMC, NextEra Energy, Microsoft, and Anthropic. Its source snapshot must establish at least two delivered evidentiary origins for every selected entity before the first model call. Calibration artifacts remain non-publishing candidates; expanding back to all 20 entities requires a separate cohort trigger after the targeted repeat results are reviewed.
...
Non-publishing cohort outcomes and methodology decisions are recorded in [`docs/ai-power-v2-pilot-run-log.md`](./ai-power-v2-pilot-run-log.md).
